...
long long totalCombination(vector<int> coins,long n,long w){
   vector<vector<long>> dp(n+1,vector<long>(w+1,0));
    
    //no. of combination when wt=0;
    for(int i=0;i<n+1;i++){
      dp[i][0]=1;
    }
    ...
    for(int i=1;i<n+1;i++){
      for(int j=1;j<w+1;j++){
        ...
        if(coins[i-1]<=j){ //case of inclusion
            dp[i][j]=dp[i][j-coins[i-1]]+dp[i-1][j]; //sum of inclusion and exclusion
        }
        else{
          dp[i][j]=dp[i-1][j];
        }
...
long long ans=totalCombination(coins,n,W);
cout<<ans<<endl;
